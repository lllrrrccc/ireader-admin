...
function toggleNoAudioFilter(checkbox) {
    const showOnlyNoAudio = checkbox && checkbox.checked;
    const tbody = document.getElementById('word-edit-tbody');
    if (!tbody) return;
    const rows = tbody.querySelectorAll('.word-row');
    rows.forEach(row => {
        const hasAudio = row.getAttribute('data-audio') !== 'false';
        if (showOnlyNoAudio) {
            if (hasAudio) {
                row.style.display = 'none';
                // Also hide subsequent meaning-rows for this word
                let next = row.nextElementSibling;
                while (next && next.classList.contains('meaning-row')) {
...
function markTimestamp(id) {
    ...
    if (audioEl && !isNaN(audioEl.currentTime)) {
        ...
        // 模拟递增时间（原型演示用）
        const now = new Date();
        seconds = (now.getMinutes() * 60 + now.getSeconds() + now.getMilliseconds() / 1000) % 3600;
    }
    ...
    timestampEl.textContent = `${pad2(hrs)}:${pad2(mins)}:${pad2(secs)}.${pad3(ms)}`;
    timestampEl.style.color = 'var(--primary)';
